...
# Print attributes of a Document instance to understand its structure
        #if len(self.processor.pages) > 0:
        #    first_document = self.processor.pages[0]
         #   st.write(dir(first_document))
          #  st.write(first_document)

        documents = [page.page_content for page in self.processor.pages]  # Accessing the page_content attribute directly
        ...
        if texts is not None:
            st.success(f"Successfully split pages into {len(texts)} documents! ✅")
        ...
        chroma_collection = Chroma.from_documents(documents=texts, embedding=self.embed_model)
